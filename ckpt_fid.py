...
def fid_ares(ckpt_name,ckpt_dir, g, r, inception, log):
    log("load model: "+ckpt_dir+'--->'+ckpt_name)
    ckpt = torch.load(os.path.join(ckpt_dir,ckpt_name), map_location=lambda storage, loc: storage)
...
if __name__ == "__main__":
    ...
    save_name = args.ckpt_dir.split('/')[-2] + '_' + args.ckpt_dir.split('/')[-1]
    ...
    ckpt_list = [i for i in os.listdir(args.ckpt_dir) if i.endswith('.pt')]
    ...
    for ckpt_name in tqdm(ckpt_list,desc='all ckpts'):
        fid_list = fid_ares(ckpt_name,args.ckpt_dir, g_ema, render, inception, log)
        wandb.log({
            "fid_256": fid_list[0],
            "fid_512": fid_list[1],
            "fid_1024": fid_list[2],
